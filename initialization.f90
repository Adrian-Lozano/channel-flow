...
Contains
  ...
  Subroutine initialize
    ...
    If (myid==0) Then 
       Write(*,*) '----------------------------------------------------------------------'       
       Write(*,*) ' '
       Write(*,*) '             My channel ^^, parallel version 0.26.1                   '
       Write(*,*) ' '
       Write(*,*) '----------------------------------------------------------------------'
    End If
    ...
    !------------------------statistics---------------------------!
    Allocate (  Umean(ny),  Vmean(ny),  Wmean(ny), Pmean(ny)              )
    Allocate ( U2mean(ny), V2mean(ny), W2mean(ny), UVmean(ny), P2mean(ny) )
    Umean  = 0d0
    Vmean  = 0d0
    Wmean  = 0d0
    Pmean  = 0d0
    ...
    If ( Abs(Retau_int)>0 ) Then
       If ( x_mass_cte == 1 ) Then
          If (myid==0) Write(*,*) 'warning: initial mass flow corrected' 
          If (myid==0) Write(*,*) '   flag:',Retau_int 
          If (Retau_int == 4200) then
             Qflow_ref = 0.873791265477842d0 ! 4200
          Elseif (Retau_int == 2000) then
             Qflow_ref = 0.898245304195718d0 ! 2000
          Elseif (Retau_int == 950) then
             Qflow_ref = 0.899952805947840d0 ! 950
          Elseif (Retau_int == 550) then
             Qflow_ref = 0.899885379198784d0 ! 550
          Elseif (Retau_int == 395) then
             Qflow_ref = 0.871974131836648d0 ! 395       
          Elseif ( Retau_int<0 ) then
             Qflow_ref = -Retau_int*Qflow_x_0
          Else
             Stop 'Error: Unknown mass flow correction'
          end if
          U = Qflow_ref/Qflow_x_0*U
       Elseif ( x_mass_cte == 2 ) Then
          Ucenterline_x = freq_mult 
          If (myid==0) Write(*,*) 'U centerline :', Ucenterline_x
          If (myid==0) Write(*,*) 'y(centerline):', yg(Floor(Real(ny/2))+1)
       End If
    End If
    ...
    If (LES_model>0) Then
       ...
       Allocate( LWij    (2:nxg  ,2:nyg-1,2:nzm+2 ,6) ) ! These need one more points in each periodic direction (for filtering)
       Allocate( MWij    (2:nxg-1,2:nyg-1,2:nzm+1, 6) )
       Allocate( MWij2   (2:nxg-1,2:nyg-1,2:nzm+1, 9) )
       Allocate( SWij    (2:nxg  ,2:nyg  ,2:nzm+2 ,6) ) ! These need one more points in each direction (for filtering)
       ! Tensor buffer
       ...
       Allocate( Ufff (1:nx ,1:nyg,1:nzm+2) )
       Allocate( Vfff (1:nxg,1:ny ,1:nzm+2) )
       Allocate( Wfff (1:nxg,1:nyg,1:nz ) )
    End If

    ! Eddy viscosity
    Allocate( nu_t     (1:nxg,1:nyg,1:nzg) )
    Allocate( nu_t_hat (1:nxg,1:nyg,1:nzg) )
    Allocate( avg_nu_t    (1,1:nyg,1) )
    ...
    ! EQWM
    tau_mean_eqwm = dPdx_ref
    Allocate( temp_1(nxg, 1, nzg), temp_2(nx, 1, nzg), temp_3(nxg, 1, nz), temp_4(nx, 1, nzg) )
    Allocate( alpha_z_temp    (1:nx,1:2,1:nzg ) )    
    Allocate( tau_eqwm_initial(1:nx,1:2,1:nzg ) )
    tau_eqwm_initial = tau_mean_eqwm
