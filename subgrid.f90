...
Contains
  ...
  Subroutine compute_eddy_viscosity(U_,V_,W_,avg_nu_t_,nu_t_)
    ...
    Elseif ( LES_model==3 ) Then
       ! AMD 
       Call sgs_AMD(U_,V_,W_,avg_nu_t_,nu_t_) 
    Else
       ! No explicit model
       nu_t_      = 0d0
       avg_nu_t_ = 0d0
  ...
  ! Output: nu_t_, avg_nu_t (eddy viscosity and average in xz) !
  !                                                            !
  !------------------------------------------------------------!
  Subroutine sgs_dynamic_Smagorinsky(U_,V_,W_,avg_nu_t_,nu_t_)
    ...
    ! local variables
    Real   (Int64), Dimension(1,nyg,1) :: LijMij_local, MijMij_local, LijMij, MijMij
    Integer(Int32) :: i, j, k
    ...
    End Do
    Call MPI_Allreduce(LijMij_local,LijMij,nyg,MPI_real8,MPI_sum,MPI_COMM_WORLD,ierr)
    ...
    ! compute <Mij*Mij>  -> MijMij_local
    MijMij_local = 0d0
    Do j = 2,nyg-1
       MijMij_local(1,j,1) = Sum( nu_t_(2:nxg-1,j,2:nzg-1) )
    End Do
    Call MPI_Allreduce(MijMij_local,MijMij,nyg,MPI_real8,MPI_sum,MPI_COMM_WORLD,ierr)

    ! compute <Lij*Mij>/<Mij*Mij> -> avg_nu_t_
    Do j = 2,nyg-1
       avg_nu_t_(1,j,1) = LijMij(1,j,1) / MijMij(1,j,1) 
    End Do

    ! compute -0.5*<Lij*Mij>/<Mij*Mij>*|S|
    Do j = 2,nyg-1
       nu_t_(2:nxg-1,j,2:nzg-1) = -0.5d0 *avg_nu_t_(1,j,1) * S(2:nxg-1,j,2:nzg-1) 
    End Do
    ...
    Call Mpi_bcast( avg_nu_t_(1,2:nyg-1,1),nyg-2,MPI_real8,0,MPI_COMM_WORLD,ierr )
  ...
  !                                                            !
  !------------------------------------------------------------!
  Subroutine sgs_AMD(U_,V_,W_,avg_nu_t_,nu_t_)
    ...
    ! term_4 = (d_i     U) * (d_i     U)
    Do i = 2,nx
      term_2(i,:,:) = (U_(i,:,:)-U_(i-1,:,:))**2d0 
      term_4(i,:,:) = (U_(i,:,:)-U_(i-1,:,:))**2d0 / (x(i)-x(i-1))**2d0
    End Do
    nu_t_(2:nxg,2:nyg,2:nzg) = term_2(2:nxg,2:nyg,2:nzg) * Sij(:,:,:,1) 

    Do j = 2,nyg
      term  (1:nx,j,:) = (U_(:,j,:)-U_(:,j-1,:))**2d0
      term_1(1:nx,j,:) = term(1:nx,j,:) / (yg(j) - yg(j-1))**2d0
    End Do
    Call interpolate_x(term_1(1:nx,2:nyg,:),term_2(2:nx,2:nyg  ,:),1)
    Call interpolate_y(term_2(2:nx,2:nyg,:),term_3(2:nx,2:nyg-1,:),1) 
    term_4 = term_4 + term_3
    Call interpolate_x(term  (1:nx,2:nyg,:),term_1(2:nx,2:nyg  ,:),1)
    Call interpolate_y(term_1(2:nx,2:nyg,:),term_2(2:nx,2:nyg-1,:),1) 
    nu_t_(2:nxg,2:nyg,2:nzg) = nu_t_(2:nxg,2:nyg,2:nzg) + term_2(2:nxg,2:nyg,2:nzg) * Sij(:,:,:,1) 

    Do k = 2,nzg
      term  (1:nx,:,k) = (U_(:,:,k)-U_(:,:,k-1))**2d0
      term_1(1:nx,:,k) = term(1:nx,:,k) / (zg(k)-zg(k-1))**2d0 
    End Do
    ...
    Do j = 2,ny
      term_2(:,j,:) = (V_(:,j,:)-V_(:,j-1,:))**2d0 
      term_4(:,j,:) = term_4(:,j,:) + term_2(:,j,:) / (y(j)-y(j-1))**2d0 
    End Do
    nu_t_(2:nxg,2:nyg,2:nzg) = nu_t_(2:nxg,2:nyg,2:nzg) + term_2(2:nxg,2:nyg,2:nzg) * Sij(:,:,:,2) 

    Do i = 2,nxg
      term  (i,1:ny,:) = (V_(i,:,:)-V_(i-1,:,:))**2d0
      term_1(i,1:ny,:) = term(i,1:ny,:) / (xg(i) - xg(i-1))**2d0
    End Do
    ...
    Do k = 2,nzg
      term  (:,1:ny,k) = (V_(:,:,k)-V_(:,:,k-1))**2d0
      term_1(:,1:ny,k) = term(:,1:ny,k) / (zg(k) - zg(k-1))**2d0 
    End Do
    ...
    Do i = 2,nxg
      term  (i,:,1:nz) = (W_(i,:,:)-W_(i-1,:,:))**2d0
      term_1(i,:,1:nz) = term(i,:,1:nz) / (xg(i) - xg(i-1))**2d0
    End Do
    ...
    Do j = 2,nyg
      term  (:,j,1:nz) = (W_(:,j,:)-W_(:,j-1,:))**2d0
      term_1(:,j,1:nz) = term(:,j,1:nz) / (yg(j) - yg(j-1))**2d0 
    End Do
    ...
    ! wall-parallel average eddy viscosity (not used)
    Do j = 2, nyg-1
       avg_nu_t_(1,j,1) = Sum( nu_t_(2:nxg-2,j,2:nzg-2) )/Real( (nxg-3)*(nzg-3), 8) 
    End Do
       ...
       Do jj = 1, 6
          ...
          Do i = 2, nxg-1
             ...
             Sij_(i,j,k,5) = 1d0/8d0*( &
                  ...
                  !-------------j-1
                  !(i,k)
                  0.5d0 * ((U_(i-1,j-1,k)-U_(i-1,j-1,k-1))/(zg(k)-zg(k-1)) + (W_(i,j-1,k-1)-W_(i-1,j-1,k-1))/(xg(i)-xg(i-1))) + &  
                  !(i+1,k)
                  0.5d0 * ((U_(i-1+1,j-1,k)-U_(i-1+1,j-1,k-1))/(zg(k)-zg(k-1)) + (W_(i+1,j-1,k-1)-W_(i-1+1,j-1,k-1))/(xg(i+1)-xg(i-1+1))) + &  
                  !(i,k+1)
                  0.5d0 * ((U_(i-1,j-1,k+1)-U_(i-1,j-1,k-1+1))/(zg(k+1)-zg(k-1+1)) + (W_(i,j-1,k-1+1)-W_(i-1,j-1,k-1+1))/(xg(i)-xg(i-1))) + &  
                  !(i+1,k+1)
                  0.5d0 * ((U_(i-1+1,j-1,k+1)-U_(i-1+1,j-1,k-1+1))/(zg(k+1)-zg(k-1+1)) + (W_(i+1,j-1,k-1+1)-W_(i-1+1,j-1,k-1+1))/(xg(i+1)-xg(i-1+1))) )
    ...
    Do k = 2, nzg-1
       Do jj = 1, 6
          ...
          Do i = 2, nxg-1
             ...
             ! 1/2*(dU/dy - dV/dx)
                                     !(i,j)
             Rij_(i,j,k,4) = 0.5d0*( 0.5d0 * ((U_(i-1,j,k)-U_(i-1,j-1,k))/(yg(j)-yg(j-1)) - (V_(i,jv-1,k)-V_(i-1,jv-1,k))/(xg(i)-xg(i-1))) + & 
                                     !(i+1,j)
                                     0.5d0 * ((U_(i-1+1,j,k)-U_(i-1+1,j-1,k))/(yg(j)-yg(j-1)) - (V_(i+1,jv-1,k)-V_(i-1+1,jv-1,k))/(xg(i+1)-xg(i-1+1))) )   
             ...
             ! 1/2*(dV/dz - dW/dy) 
                                     !(j,k)
             Rij_(i,j,k,6) = 0.5d0*( 0.5d0 * ((V_(i,jv-1,k)-V_(i,jv-1,k-1))/(zg(k)-zg(k-1)) - (W_(i,j,k-1)-W_(i,j-1,k-1))/(yg(j)-yg(j-1))) + & 
                                     !(j,k+1)
                                     0.5d0 * ((V_(i,jv-1,k+1)-V_(i,jv-1,k-1+1))/(zg(k+1)-zg(k-1+1)) - (W_(i,j,k-1+1)-W_(i,j-1,k-1+1))/(yg(j)-yg(j-1))) ) 
          End Do
       End Do
