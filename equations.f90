    ...
    Do k=2,nzg-1
       ...
       Do j=2,nyg-1
          ...
          Do i=2,nx-1
             ...
             ! viscous term
             term(i,j,k) = 2d0/dx_3*(nu_x2*(U_(i+1,j,k)-U_(i,j,k))/dx_2 - nu_x1*(U_(i,j,k)-U_(i-1,j,k))/dx_1 )   + & !d(2(nu+nu_t)*du/dx)/dx
                           1d0/dy_3*(nu_y2*( term_2(i,j  ,k) + (V_(i+1,j  ,k)-V_(i,j  ,k))/dx_2 )                - &
                                     nu_y1*( term_2(i,j-1,k) + (V_(i+1,j-1,k)-V_(i,j-1,k))/dx_1 ) )              + & !d((nu+nu_t)*(du/dy+dv/dx))/dy
                           1d0/dz_3*(nu_z2*( (U_(i,j,k+1)-U_(i,j,k  ))/dz_2 + (W_(i+1,j,k  )-W_(i,j,k  ))/dx_2 ) - & 
                                     nu_z1*( (U_(i,j,k  )-U_(i,j,k-1))/dz_1 + (W_(i+1,j,k-1)-W_(i,j,k-1))/dx_1 ) )   !d((nu+nu_t)*(du/dz+dw/dx))/dz
  ...
  Subroutine compute_rhs_v(U_,V_,W_,rhs_v)
    ...
    Do k=2,nzg-1
       ...
       Do j=2,ny-1
          ...
          dy_1 = y(  j) - y(j-1) 
          dy_2 = y(j+1) - y(j  )
          ! second/first derivation in y/yg (V/U at centers)
          dy_3 = yg(j+1) - yg(j)          
          ...
          Do i=2,nxg-1
             ...
             ! viscous term
             term_2(i,j,k) = 1d0/dx_3*(nu_x2*( (V_(i+1,j,k) - V_(i  ,j,k))/dx_2 + (U_(i  ,j+1,k) - U_(i  ,j,k))/dy_3 )   - & 
                                       nu_x1*( (V_(i,  j,k) - V_(i-1,j,k))/dx_1 + (U_(i-1,j+1,k) - U_(i-1,j,k))/dy_3 ) ) + & !d((nu+nu_t)(dv/dx+du/dy))/dx 
                             2d0/dy_3*(nu_y2*(V_(i,j+1,k) - V_(i,j  ,k))/dy_2 - nu_y1*(V_(i,j  ,k) - V_(i,j-1,k))/dy_1 ) + & !d(2(nu+nu_t)dv/dy)/dy
                             1d0/dz_3*(nu_z2*( (V_(i,j,k+1) - V_(i,j,k  ))/dz_2 + (W_(i,j+1,k  ) - W_(i,j,k  ))/dy_3 )   - & 
                                       nu_z1*( (V_(i,j,k  ) - V_(i,j,k-1))/dz_1 + (W_(i,j+1,k-1) - W_(i,j,k-1))/dy_3 ) )     !d((nu+nu_t)(dv/dz+dw/dy))/dz
    ...
    Do k=2,nz-1
       ...
       Do j=2,nyg-1
          ...
          Do i=2,nxg-1
             ...
             ! viscous term
             term(i,j,k) = 1d0/dx_3*(nu_x2*( (W_(i+1,j,k) - W_(i  ,j,k))/dx_2 + (U_(i  ,j,k+1) - U_(i  ,j,k))/dz_2 )     - & 
                                     nu_x1*( (W_(i  ,j,k) - W_(i-1,j,k))/dx_1 + (U_(i-1,j,k+1) - U_(i-1,j,k))/dz_1 ) )   + & !d((nu+nu_t)*(dw/dx+du/dz))/dx
                           1d0/dy_3*(nu_y2*( term_2(i,j  ,k) + (V_(i,j  ,k+1) - V_(i,j  ,k))/dz_2 )                      - & 
                                     nu_y1*( term_2(i,j-1,k) + (V_(i,j-1,k+1) - V_(i,j-1,k))/dz_1 ) )                    + & !d((nu+nu_t)*(dw/dy+dv/dz))/dy
                           2d0/dz_3*(nu_z2*(W_(i,j,k+1)-W_(i,j,k))/dz_2 - nu_z1*(W_(i,j,k)-W_(i,j,k-1))/dz_1 )               !d(2(nu+nu_t)*dw/dz)/dz
