...
Contains
  ...
  Subroutine compute_time_step_Euler
    ...
    Call compute_wall_model(Uo,Vo,Wo,nu_t)
    ! compute rhs for U
  ...
  Subroutine compute_time_step_RK3
    ...
    Call compute_eddy_viscosity(U,V,W,avg_nu_t,nu_t) ! this is just in case I do wrong compute_wall_model()
    ...
    Elseif ( x_mass_cte == 2 ) Then
       Call compute_dPx_for_constant_Ucenterline(U,dPdx)
       U(2:nx-1,2:nyg-1,2:nzg-1) = U(2:nx-1,2:nyg-1,2:nzg-1) + dPdx
       dPdx = dPdx/dt ! to be used later by rhs_*
       Call apply_boundary_conditions
    End If
